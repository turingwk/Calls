...
public class RongIM {
        ...
        if(!mainProcessName.equals(current)) {
            ...
            if(TextUtils.isEmpty(RongIM.SingletonHolder.sRongIM.mAppKey)) {
                try {
                    ...
                    if(e != null) {
                        RongIM.SingletonHolder.sRongIM.mAppKey = e.metaData.getString("RONG_CLOUD_APP_KEY");
                    }
    ...
    public static void init(Context context, String appKey) {
        ...
        if(!mainProcessName.equals(current)) {
            ...
            RongIM.SingletonHolder.sRongIM.mAppKey = appKey;
            init(context);
        }
    }
    ...
    public static RongIM connect(final String token, final ConnectCallback callback) {
        if(RongContext.getInstance() == null) {
            RLog.e(TAG, "connect should be called in main process.");
            return RongIM.SingletonHolder.sRongIM;
        } else {
            saveToken(token);
            RongUserInfoManager.getInstance().init(mContext, RongIM.SingletonHolder.sRongIM.mAppKey, new RongUserCacheListener());
    ...
    private static void initListener() {
        ...
        RongIMClient.setOnRecallMessageListener(new OnRecallMessageListener() {
            public boolean onMessageRecalled(Message message, RecallNotificationMessage recallNotificationMessage) {
                RongContext.getInstance().getEventBus().post(new RemoteMessageRecallEvent(message.getMessageId(), message.getConversationType(), recallNotificationMessage, true));
        ...
        if(context != null && targetUserIds != null && targetUserIds.size() != 0) {
            RongIMClient.getInstance().createDiscussion(title, targetUserIds, new CreateDiscussionCallback() {
                public void onSuccess(String targetId) {
                    Uri uri = Uri.parse("rong://" + context.getApplicationInfo().packageName).buildUpon().appendPath("conversation").appendPath(ConversationType.DISCUSSION.getName().toLowerCase()).appendQueryParameter("targetIds", TextUtils.join(",", targetUserIds)).appendQueryParameter("delimiter", ",").appendQueryParameter("targetId", targetId).appendQueryParameter("title", title).build();
                    context.startActivity(new Intent("android.intent.action.VIEW", uri));
                }
    ...
    public void startDiscussionChat(Context context, String targetDiscussionId, String title) {
        if(context != null && !TextUtils.isEmpty(targetDiscussionId)) {
            Uri uri = Uri.parse("rong://" + context.getApplicationInfo().packageName).buildUpon().appendPath("conversation").appendPath(ConversationType.DISCUSSION.getName().toLowerCase()).appendQueryParameter("targetId", targetDiscussionId).appendQueryParameter("title", title).build();
            context.startActivity(new Intent("android.intent.action.VIEW", uri));
        } else {
            throw new IllegalArgumentException();
        }
    ...
    public void startGroupChat(Context context, String targetGroupId, String title) {
        if(context != null && !TextUtils.isEmpty(targetGroupId)) {
            if(RongContext.getInstance() == null) {
                ...
                Uri uri = Uri.parse("rong://" + context.getApplicationInfo().packageName).buildUpon().appendPath("conversation").appendPath(ConversationType.GROUP.getName().toLowerCase()).appendQueryParameter("targetId", targetGroupId).appendQueryParameter("title", title).build();
                context.startActivity(new Intent("android.intent.action.VIEW", uri));
            }
        } else {
            throw new IllegalArgumentException();
    ...
    public void startChatRoomChat(Context context, String chatRoomId, boolean createIfNotExist) {
        if(context != null && !TextUtils.isEmpty(chatRoomId)) {
            if(RongContext.getInstance() == null) {
                ...
                Uri uri = Uri.parse("rong://" + context.getApplicationInfo().packageName).buildUpon().appendPath("conversation").appendPath(ConversationType.CHATROOM.getName().toLowerCase()).appendQueryParameter("targetId", chatRoomId).build();
                Intent intent = new Intent("android.intent.action.VIEW", uri);
    ...
    public void startCustomerServiceChat(Context context, String customerServiceId, String title, CSCustomServiceInfo customServiceInfo) {
        if(context != null && !TextUtils.isEmpty(customerServiceId)) {
            if(RongContext.getInstance() == null) {
                ...
                Uri uri = Uri.parse("rong://" + context.getApplicationInfo().packageName).buildUpon().appendPath("conversation").appendPath(ConversationType.CUSTOMER_SERVICE.getName().toLowerCase()).appendQueryParameter("targetId", customerServiceId).appendQueryParameter("title", title).build();
                Intent intent = new Intent("android.intent.action.VIEW", uri);
    ...
    public void startPublicServiceProfile(Context context, ConversationType conversationType, String targetId) {
        if(context != null && conversationType != null && !TextUtils.isEmpty(targetId)) {
            if(RongContext.getInstance() == null) {
                ...
                Uri uri = Uri.parse("rong://" + context.getApplicationInfo().processName).buildUpon().appendPath("publicServiceProfile").appendPath(conversationType.getName().toLowerCase()).appendQueryParameter("targetId", targetId).build();
                Intent intent = new Intent("android.intent.action.VIEW", uri);
        ...
        RongIMClient.getInstance().removeConversation(type, targetId, new ResultCallback<Boolean>() {
            public void onSuccess(Boolean bool) {
                ...
                if(bool.booleanValue()) {
                    RongContext.getInstance().removeConversationNotifyStatusFromCache(ConversationKey.obtain(targetId, type));
    ...
    public boolean deleteMessages(int[] messageIds) {
        Boolean bool = Boolean.valueOf(RongIMClient.getInstance().deleteMessages(messageIds));
        if(bool.booleanValue()) {
            ...
            Message message = RongIMClient.getInstance().sendMessage(type, targetId, content, pushContent, pushData, new SendMessageCallback() {
                public void onSuccess(Integer messageId) {
                    if(result.t != null) {
                        ...
                        long tt = RongIMClient.getInstance().getSendTimeByMessageId(messageId.intValue());
                        if(tt != 0L) {
    ...
    public void sendImageMessage(Message message, String pushContent, String pushData, final SendImageMessageCallback callback) {
        Message temp = this.filterSendMessage(message);
        if(temp != null) {
            if(temp != message) {
                message = temp;
    ...
    private void filterSentMessage(Message message, ErrorCode errorCode) {
        ...
        if(errorCode != null && !isExecute) {
            if(errorCode.equals(ErrorCode.NOT_IN_DISCUSSION) || errorCode.equals(ErrorCode.NOT_IN_GROUP) || errorCode.equals(ErrorCode.NOT_IN_CHATROOM) || errorCode.equals(ErrorCode.REJECTED_BY_BLACKLIST) || errorCode.equals(ErrorCode.FORBIDDEN_IN_GROUP) || errorCode.equals(ErrorCode.FORBIDDEN_IN_CHATROOM) || errorCode.equals(ErrorCode.KICKED_FROM_CHATROOM)) {
                InformationNotificationMessage content1 = null;
                if(errorCode.equals(ErrorCode.NOT_IN_DISCUSSION)) {
                    content1 = InformationNotificationMessage.obtain(mContext.getString(string.rc_info_not_in_discussion));
                } else if(errorCode.equals(ErrorCode.NOT_IN_GROUP)) {
                    content1 = InformationNotificationMessage.obtain(mContext.getString(string.rc_info_not_in_group));
                } else if(errorCode.equals(ErrorCode.NOT_IN_CHATROOM)) {
                    content1 = InformationNotificationMessage.obtain(mContext.getString(string.rc_info_not_in_chatroom));
                } else if(errorCode.equals(ErrorCode.REJECTED_BY_BLACKLIST)) {
                    content1 = InformationNotificationMessage.obtain(mContext.getString(string.rc_rejected_by_blacklist_prompt));
                } else if(errorCode.equals(ErrorCode.FORBIDDEN_IN_GROUP)) {
                    content1 = InformationNotificationMessage.obtain(mContext.getString(string.rc_info_forbidden_to_talk));
                } else if(errorCode.equals(ErrorCode.FORBIDDEN_IN_CHATROOM)) {
                    content1 = InformationNotificationMessage.obtain(mContext.getString(string.rc_forbidden_in_chatroom));
                } else if(errorCode.equals(ErrorCode.KICKED_FROM_CHATROOM)) {
                    content1 = InformationNotificationMessage.obtain(mContext.getString(string.rc_kicked_from_chatroom));
                }

                this.insertMessage(message.getConversationType(), message.getTargetId(), "rong", content1, new ResultCallback<Message>() {
                    public void onSuccess(Message message) {
                    }
